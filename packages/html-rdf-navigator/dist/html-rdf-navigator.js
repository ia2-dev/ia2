...
${t}`)n^=r.codePointAt(0)??0,n=Math.imul(n,16777619);return`BlankNodeQuery:${(n>>>0).toString(16)}`}function Te(o){let e=new Map,t=a=>{let i=Ct(a),s=e.get(i);return s||(s={executable:!1,queries:{},subject:a},e.set(i,s)),s};for(let a of o.quads){let i=t(a.subject);if(a.predicate.value===Ln&&a.object.termType==="NamedNode"&&Nn.has(a.object.value)&&(i.executable=!0),a.object.termType!=="Literal")continue;let s=Nt.find(({iri:c})=>c===a.predicate.value);if(s&&(i.queries[s.kind]=a.object.value.trim()),[Rn,Tn,`${J}description`].includes(a.predicate.value)&&(i.description??=a.object.value.trim()),a.predicate.value===`${J}order`){let c=Number(a.object.value);Number.isFinite(c)&&(i.order=c)}}let n=[],r=Array.from(e.values()).flatMap(a=>{if(!a.executable)return[];let i=Nt.map(({kind:c})=>({kind:c,query:a.queries[c]})).filter(c=>!!c.query);if(i.length!==1)return n.push(`${Mt(a.subject)} must declare exactly one sh:select, sh:ask, or sh:construct query.`),[];let s=i[0];return[{description:a.description??"",id:Mn(a.subject,s.kind,s.query),kind:s.kind,label:ne(o.quads,a.subject,{predicates:[...ye,`${J}name`]})?.trim()||Mt(a.subject),order:a.order??Number.POSITIVE_INFINITY,query:s.query}]}).sort((a,i)=>a.order-i.order||a.label.localeCompare(i.label));return{diagnostics:n,queries:r}}function Cn(o){return Te(o).queries}var Dn="http://www.w3.org/1999/02/22-rdf-syntax-ns#type",qn="http://www.w3.org/1999/02/22-rdf-syntax-ns#Property",An="http://www.w3.org/2000/01/rdf-schema#Class",In="http://www.w3.org/2000/01/rdf-schema#subClassOf",Pn="http://www.w3.org/2000/01/rdf-schema#subPropertyOf",$n=new Set([An,"http://www.w3.org/2002/07/owl#Class","http://www.w3.org/2002/07/owl#DeprecatedClass"]),Hn=new Set([qn,"http://www.w3.org/2002/07/owl#ObjectProperty","http://www.w3.org/2002/07/owl#DatatypeProperty","http://www.w3.org/2002/07/owl#AnnotationProperty","http://www.w3.org/2002/07/owl#FunctionalProperty","http://www.w3.org/2002/07/owl#InverseFunctionalProperty","http://www.w3.org/2002/07/owl#TransitiveProperty","http://www.w3.org/2002/07/owl#SymmetricProperty","http://www.w3.org/2002/07/owl#AsymmetricProperty","http://www.w3.org/2002/07/owl#ReflexiveProperty","http://www.w3.org/2002/07/owl#IrreflexiveProperty","http://www.w3.org/2002/07/owl#DeprecatedProperty","http://www.w3.org/2002/07/owl#OntologyProperty"]);function Qe(o,e){o.some(t=>t.value===e.value)||o.push(e)}function Ge(o,e){o.includes(e)||o.push(e)}function Re(o,e){o.includes(e)||o.push(e)}function Ye(o){let e=new Map,t=i=>{let s=e.get(i.value);return s||(s={classParents:[],kinds:[],propertyParents:[],sources:[],term:i,types:[]},e.set(i.value,s)),s};for(let i of o.quads)if(i.subject.termType==="NamedNode"){if(i.predicate.value===Dn&&i.object.termType==="NamedNode"){let s=$n.has(i.object.value),c=Hn.has(i.object.value);if(!s&&!c)continue;let l=t(i.subject);s&&Re(l.kinds,"class"),c&&Re(l.kinds,"property"),Qe(l.types,i.object),Ge(l.sources,i.source);continue}if(i.predicate.value===In){let s=t(i.subject);Re(s.kinds,"class"),i.object.termType==="NamedNode"&&Qe(s.classParents,i.object),Ge(s.sources,i.source);continue}if(i.predicate.value===Pn){let s=t(i.subject);Re(s.kinds,"property"),i.object.termType==="NamedNode"&&Qe(s.propertyParents,i.object),Ge(s.sources,i.source)}}let n=Array.from(e.values()).map(i=>{let s=ne(o.quads,i.term);return{...i,...s?{label:s}:{}}}).sort((i,s)=>(i.label??i.term.value).localeCompare(s.label??s.term.value)),r=n.filter(i=>i.kinds.includes("class")),a=n.filter(i=>i.kinds.includes("property"));return{classes:r,count:n.length,definitions:n,properties:a}}var zn=String.raw`
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  ${ht}
`,jn={navigator:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><circle cx="3" cy="5" r=".8" fill="currentColor" stroke="none"/><circle cx="3" cy="9" r=".8" fill="currentColor" stroke="none"/><circle cx="3" cy="13" r=".8" fill="currentColor" stroke="none"/><path d="M6 5h9M6 9h9M6 13h9"/></svg>',sources:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><rect x="2.5" y="3" width="13" height="9" rx="1.5"/><path d="M6 15h6M9 12v3"/></svg>',vocabulary:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><circle cx="9" cy="3.5" r="2"/><circle cx="4" cy="14" r="2"/><circle cx="14" cy="14" r="2"/><path d="M9 5.5v3M4 12V9h10v3"/></svg>',shapes:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path d="M3 3.5h5v5H3zM10 9.5h5v5h-5zM8 6h3v3.5"/><path d="m4.3 11.8 1.3 1.3 2.6-3"/></svg>',discovery:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><circle cx="9" cy="9" r="6.5"/><path d="m11.7 6.3-1.5 3.9-3.9 1.5 1.5-3.9z"/></svg>',sparql:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path d="M3 4.5h8M3 9h6M3 13.5h5"/><circle cx="13" cy="12" r="3"/><path d="m15.2 14.2 1.5 1.5"/></svg>',turtle:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path d="m6.5 4.5-4 4.5 4 4.5M11.5 4.5l4 4.5-4 4.5"/></svg>',json:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path d="M7 3.5H5.5c-1 0-1.5.5-1.5 1.5v2c0 1-.5 1.5-1.5 2 1 .5 1.5 1 1.5 2v2c0 1 .5 1.5 1.5 1.5H7M11 3.5h1.5c1 0 1.5.5 1.5 1.5v2c0 1 .5 1.5 1.5 2-1 .5-1.5 1-1.5 2v2c0 1-.5 1.5-1.5 1.5H11"/></svg>',diagnostics:'<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path d="M8 3.2 2.3 13a1.2 1.2 0 0 0 1 1.8h11.4a1.2 1.2 0 0 0 1-1.8L10 3.2a1.15 1.15 0 0 0-2 0Z"/><path d="M9 6.8v3.4M9 13h.01"/></svg>'};function ae(o,e,t,n,r,a){let i=r===void 0?t:`${t} (${r})`,s=r===void 0||!a?t:`${t}, ${r} ${a}${r===1?"":"s"}`;return`<button class="tab" role="tab" data-view="${o}" aria-selected="${e}" aria-label="${i}" title="${s}"><span class="tab-icon" aria-hidden="true">${jn[o]}</span><span class="tab-label" data-short="${n}">${t}</span>${r===void 0?"":`<span class="tab-count"> (${r})</span>`}</button>`}var Dt="ia2:rdf-navigator:state:v1",Xe=`SELECT ?subject ?predicate ?object ?graph
WHERE {
  {
      ...
      <aside class="panel ia2-window-surface" id="ia2-rdf-panel" data-open="${this.#n}" data-position="${this.#c}" aria-label="Document RDF" tabindex="-1">
        <header class="toolbar">
          ...
          <div class="tabs" role="tablist" aria-label="RDF views" data-compact="0">
            ...
            ${this.#k.count?ae("vocabulary",this.#e==="vocabulary","Vocabulary","Vocab",this.#k.count,"definition"):""}
            ${this.#v.count?ae("shapes",this.#e==="shapes","Shapes","Shapes",this.#v.count,"shape"):""}
            ${this.#S.length?ae("discovery",this.#e==="discovery","Discovery","Discover",this.#S.length,"candidate"):""}
            ${ae("sparql",this.#e==="sparql","SPARQL","Query",this.#u.length||void 0,"suggested query")}
